...
function heatmapCellsSvg({ trainingDates = [], dailyStats = [], startDate, endDate, x = 0, y = 0, cell = 18, gap = 7, columns = 53, maxWidth, accent = '#D7FF4B', text = '#F6F7F2' } = {}) {
  ...
  const safeGap = maxWidth ? Math.max(2, Math.min(gap, maxWidth / (visibleColumns * 6))) : gap;
  const safeCell = maxWidth
    ? Math.max(3, Math.min(cell, (maxWidth - (visibleColumns - 1) * safeGap) / visibleColumns))
    : cell;
...
function renderRichInfographicSvg({ canvas, token, eyebrow, title, subtitle, metrics, footer, trendPoints = [], trainingDates = [], dailyStats = [], dateStart, dateEnd, bodyDistribution = [] }) {
  ...
  const radarLabels = radarItems.length >= 3 ? radarItems : [];
  ...
  const radarGrid = n ? [0.33, 0.66, 1].map((scale) => `<polygon points="${radarLabels.map((_, index) => point(index, scale).join(',')).join(' ')}" fill="none" stroke="${text}" stroke-opacity=".16"/>`).join('') : '';
  const radarAxes = n ? radarLabels.map((_, index) => { const [x, y] = point(index, 1); return `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="${text}" stroke-opacity=".14"/>`; }).join('') : '';
  const radarShape = n ? `<polygon points="${radarLabels.map((item, index) => point(index, Math.max(0.08, (Number(item.value) || 0) / radarMax)).join(',')).join(' ')}" fill="${accent}" fill-opacity=".26" stroke="${accent}" stroke-width="5"/>` : '';
  const radarText = n ? radarLabels.map((item, index) => { const [x, y] = point(index, 1.18); return `<text x="${x}" y="${y}" text-anchor="middle" font-family="Microsoft YaHei, Noto Sans CJK SC, PingFang SC, sans-serif" font-size="18" fill="${muted}">${escapeXml(item.label)}</text>`; }).join('') : `<text x="${cx}" y="${cy + 8}" text-anchor="middle" font-family="${sans}" font-size="20" fill="${muted}" data-radar="no-data">暂无足够部位数据</text>`;
  ...
  const heatmapX = Math.round(canvas.width * 0.53);
  const heatmapWidth = Math.round(canvas.width * 0.39);
  const gridWidth = Math.round(canvas.width * 0.34);
  const gridX = heatmapX + Math.round((heatmapWidth - gridWidth) / 2);
  const parsedStart = dateStart ? new Date(`${dateStart}T00:00:00Z`) : null;
  const parsedEnd = dateEnd ? new Date(`${dateEnd}T00:00:00Z`) : null;
  const crossYear = parsedStart && parsedEnd && !Number.isNaN(parsedStart.getTime()) && !Number.isNaN(parsedEnd.getTime()) && parsedStart.getUTCFullYear() !== parsedEnd.getUTCFullYear();
  const cells = crossYear
    ? `<text x="${gridX + gridWidth / 2}" y="${Math.round(canvas.height * 0.74)}" text-anchor="middle" font-family="${sans}" font-size="18" fill="${muted}" data-cross-year="true">跨年范围请拆分为年度面板</text>`
    : yearHeatmapSvg({ trainingDates, dailyStats, startDate: dateStart, x: gridX, y: Math.round(canvas.height * 0.63), width: gridWidth, height: Math.round(canvas.height * 0.23), accent, text });
  const heatmap = `<rect x="${heatmapX}" y="${Math.round(canvas.height * 0.56)}" width="${heatmapWidth}" height="${Math.round(canvas.height * 0.32)}" rx="18" fill="${text}" fill-opacity=".035" stroke="${text}" stroke-opacity=".22" stroke-width="2"/><text x="${gridX}" y="${Math.round(canvas.height * 0.60)}" font-family="${serif}" font-size="24" fill="${text}">训练热力</text>${cells}`;
