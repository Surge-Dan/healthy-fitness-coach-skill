...
function buildTrainingGuidance(input = {}) {
  ...
  const safety = flags.length
    ? { level: 'stop', reason_code: 'red_flag', message: '出现需要线下评估的危险信号，先停止训练建议。' }
    : pain.length
      ? { level: 'caution', reason_code: 'pain_feedback', message: '存在疼痛或不适，先降低负荷并观察，不把疼痛当成训练效果。' }
      : { level: 'proceed', reason_code: 'no_red_flag', message: '未提供需要立即转介的危险信号。' };

  const facts = [
    { code: 'training_days', value: finite(trends.training_days) || 0, label: '有效训练天数' },
    { code: 'record_count', value: finite(trends.record_count) || 0, label: '可分析记录数' }
  ];
  ...
  if (safety.level === 'stop') {
    ...
    if (['hypertrophy', 'strength', 'upper_body', 'general_fitness'].some((term) => goal.includes(term))) {
      if ((finite(trends.training_days) || 0) < 2 || frequency <= 2) actions.push({ code: 'minimum_effective_dose', priority: 'high', text: '先安排每周2次全身或上下肢交替训练，每次保留1–3次余力，连续记录4周。' });
      actions.push({ code: 'progressive_overload', priority: 'medium', text: '在动作稳定且目标次数完成时，只增加一个变量：小幅加重、增加次数或增加一组。' });
      nextValidation.push({ code: 'log_rpe_or_rir', text: '每个主动作记录RPE或RIR、完成情况和疼痛评分。' });
    }
    if (['aerobic', 'endurance', 'cardio'].some((term) => goal.includes(term))) {
      actions.push({ code: 'easy_aerobic_progression', priority: 'high', text: '以能完整说短句的轻中等强度为主，每周增加时长或频率中的一个变量。' });
      nextValidation.push({ code: 'log_duration_and_heart_rate', text: '记录时长、距离和平均心率，避免只用主观疲劳判断进步。' });
    }
