...
function renderTrainingSummarySvg(summary = {}) {
  ...
  const metrics = [
    ['训练天数', totals.training_days || 0],
    ['训练记录', totals.sessions || 0],
    ['总组数', totals.total_sets || 0],
    ['容量kg', totals.volume_kg || 0]
  ];
  const cells = metrics.map(([label, value], index) => {
    const x = 24 + (index % 2) * 190;
    const y = 58 + Math.floor(index / 2) * 94;
    return `<g><rect x="${x}" y="${y}" width="166" height="70" rx="12" fill="#F3F0E8" stroke="#D8D1C4"/><text x="${x + 16}" y="${y + 23}" fill="#6E6A61" font-size="12">${escapeXml(label)}</text><text x="${x + 16}" y="${y + 53}" fill="#202321" font-size="24" font-weight="700">${escapeXml(value)}</text></g>`;
  }).join('');
