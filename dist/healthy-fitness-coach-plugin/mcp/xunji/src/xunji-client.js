...
function decodeXunjiResponse(input) {
  ...
  if (!payload || payload.success !== true || !Array.isArray(payload.res)) throw connectorError('invalid_response');
...
class XunjiClient {
  ...
  async fetchDay(date, credential) {
    ...
    try {
      ...
      if (error && ['invalid_credentials', 'rate_limited', 'invalid_response', 'invalid_date', 'missing_credentials'].includes(error.code)) throw error;
...
module.exports = { decodeXunjiResponse, PRODUCTION_URL, XunjiClient };
