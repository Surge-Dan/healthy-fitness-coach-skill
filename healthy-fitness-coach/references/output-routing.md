...
命令自身被否定时不触发覆盖，例如“不要进入跟练”或 “don't enter tracking”仍按任务默认模式路由。解析会规范化 Unicode 撇号、连字符、标点和空格，但只检查命令前有限的上下文：中文识别“不想/不要/不需要/无需/不必/别/拒绝”及可选“再/给我”，英文识别 `do not`、`don't`、`dont`、`do-not`、`not want`、`no need`、`refuse` 等紧邻意图短语。不会因为远处无关的 `not` 全局拒绝命令；同一句后续出现的独立、肯定命令仍可生效。
