...
function renderRichInfographicSvg({ canvas, token, eyebrow, title, subtitle, metrics, footer, trendPoints = [], trainingDates = [], dailyStats = [], dateStart, dateEnd, bodyDistribution = [] }) {
  ...
  const cells = yearHeatmapSvg({ trainingDates, dailyStats, startDate: dateStart, x: gridX, y: Math.round(canvas.height * 0.63), width: gridWidth, height: Math.round(canvas.height * 0.23), accent, text });
