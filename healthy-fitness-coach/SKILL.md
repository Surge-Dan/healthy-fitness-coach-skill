...
数据图谱提供两种布局：`rich` 信息图默认使用中文字体与更宽松的标题/正文层级，包含核心指标、趋势线、训练热力、部位/动作雷达分布和记录页脚；`minimal` 极简分享图保留更多留白，只突出一句结论和少量数字。用户未指定时，年度/月度复盘默认使用 `rich`，社交分享或战绩卡默认使用 `minimal`；用户说“丰富一点/信息图”或“简洁一点/适合发朋友圈”时按意图切换。
